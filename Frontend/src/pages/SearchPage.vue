  ...
  <div class="container">
    ...
    <div class="menu">
      ...
      <div class="flex-grow-1 mr-2">
        <b-form-input v-model="text" placeholder="Search" @focus="openRecentSearches" class="w-100"></b-form-input>
      </div>

      <!-- Radio buttons for sort type selection -->
      <b-form-group class="sort-type-radio" id="sortTypeGroup" v-slot="{ ariaDescribedby }">

        <label class="section-label">Sort By:</label>
        <b-form-radio-group class="options" id="sortTypeGroupRadios" v-model="selectedSortType" :options="sortOptions" :aria-describedby="ariaDescribedby" name="sortType"></b-form-radio-group> <br>

        <!-- Radio buttons for results per page -->
        <label class="section-label">Results Per Page:</label>
        <b-form-radio-group class="options" id="resultsPerPageGroupRadios" v-model="resultsPerPage" :options="resultsPerPageOptions" :aria-describedby="ariaDescribedby" name="resultsPerPage"></b-form-radio-group>

      </b-form-group>

      <!-- Search button -->
      <div>
        <b-button @click="search" :disabled="text.length === 0" class="btn btn-primary ml-2" size="lg" variant="success">Search</b-button>
      </div>
      ...
      <div class="col-md-4">
        ...
        <b-form-select v-model="selectedCuisine" :options="cuisineOptions" placeholder="Select Cuisine" class="mb-3"></b-form-select>
      </div>

      <!-- ################### diet ################### -->
      <div class="col-md-4">
        <label for="diet" class="highlight-label">Diet:</label>
        <b-form-select v-model="selectedDiet" :options="dietOptions" placeholder="Select Diet" class="mb-3"></b-form-select>
      </div>

      <!-- ################### intolerance ################### -->
      <div class="col-md-4">
        <label for="intolerance" class="highlight-label">Intolerance:</label>
        <b-form-select v-model="selectedIntolerance" :options="intoleranceOptions" placeholder="Select Intolerance" class="mb-3"></b-form-select>
      </div>
      </div>

      <!-- ################### recent searches ################### -->
      <div v-if="showRecentSearches && recentSearches.length > 0" class="mt-3 position-relative">
      <small class="text-danger" style="text-shadow: 2px 2px 4px rgba(248, 244, 5, 0.89); font-size: large; font-weight: bolder;">Recent Searches</small>
      ...
      <ul class="list-group mt-3" ref="recentSearchesList">
        <li v-for="search in recentSearches" :key="search" class="list-group-item recent-search-item" @click="selectRecentSearch(search)">
          {{ search }}
        </li>
      </ul>
      </div>
