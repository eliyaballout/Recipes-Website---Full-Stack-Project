<template>
  <div>
    ...
    <b-card no-body style="max-width: 20rem">
      ...
      <a style="cursor: pointer" @click="handleClick(recipe.id)">
      ...
      <b-card-body>
        ...
        <b-card-sub-title>
          ...
          <span v-if="isFamily">Made by:
            <span style="color: red; font-weight: bold;">{{recipe.owner}}</span><br>
          </span>

          <span v-if="isFamily">Prepared in:
            <span style="color: rgb(149, 0, 255); font-weight: bold;">{{recipe.whenToPrepare}}</span>
          </span> 
        ...
        <b-button v-if="!isFamily && !isMyRecipe" @click="toggleFavorite(recipe.id)" variant="link" size="sm" class="favorite-button">
        ...
        if (this.$root.store.username) {
          ...
          if (!this.isViewed && !this.isFamily && !this.isMyRecipe){
            ...
            try {
              ...
              if (response.status != 200){
                console.log("Cannot add recipe to watched list!");
              }
            } catch(error){
              console.log(error);
            }
